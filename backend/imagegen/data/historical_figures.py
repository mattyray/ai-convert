# imagegen/data/historical_figures.py
"""
Historical figure image mappings for face swap functionality.
Maps figure names to their Cloudinary URLs.
"""
...
def get_random_figure():
    """Get a random historical figure name and URL"""
    import random
    figure_name = random.choice(list(HISTORICAL_FIGURES.keys()))
    return figure_name, HISTORICAL_FIGURES[figure_name]

def get_figure_url(figure_name):
    """Get URL for a specific historical figure"""
    return HISTORICAL_FIGURES.get(figure_name)

def get_all_figures():
    """Get all available historical figures"""
    return HISTORICAL_FIGURES.copy()